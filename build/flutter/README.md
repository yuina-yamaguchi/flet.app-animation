...
* `apps_flet_animation` - project identifier (lowercase, no spaces, i.e. "snake_case") used for bundle IDs and package names.
* `` - project description.
* `apps-flet-animation` - executable or bundle name on disk.
* `apps-flet-animation` - display name shown in window titles and about app dialogs.
* `dev.flet` - org name in reverse domain name notation, e.g. `com.mycompany.myproject`.
